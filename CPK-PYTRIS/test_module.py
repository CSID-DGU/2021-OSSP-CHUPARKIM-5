...
def update_display():
    ...
    if origianl_btn.isOver_2(pos):
        ...
        origianl_btn.text = text1
    if rotate_btn.isOver_2(pos):
        rotate_btn.text = text4
    else:
    ...
    if blackout_btn.isOver_2(pos):
        blackout_btn.text = text8
    else:
        blackout_btn.text = text7
...
def draw_dual_sidebar(block_size, next, hold, next2, hold2):
    ...
    pygame.draw.line(
        screen, ui_variables.red_b, [w_4 + w_1 * 3, h_1], [w_4 + w_1 * 3, h_1 + temp], 5
    )
    pygame.draw.line(
        screen, ui_variables.red_b, [w_4, h_1 + temp], [w_4 + w_1 * 3, h_1 + temp], 5
    )
    # 보드 1
    ...
    for i in range(4):
        for j in range(4):
            dx = (
                block_size + w_s * game_loc.nh_b2_const_x + block_size * j
            )  # next mino그릴 좌표, 재지정 해줄 것
            dy = (
                block_size * game_loc.nmino_const_y + h_1 + block_size * i
            )  # next mino그릴 좌표, 재지정 해줄 것
    ...
    pygame.draw.rect(
        ...
        Rect(
            block_size + w_s,
            block_size * game_loc.scorev_const_y + h_1,
            block_size * 2,
            block_size * 2,
        ),
    )  # 스코어 중첩방지를 위해 덮어 씌워기
    ...
    screen.blit(
        text_d2_hold,
        (
            block_size + w_s * game_loc.nh_b2_const_x,
            block_size * game_loc.holdt_const_y + h_1,
        ),
    )
    screen.blit(
        text_d2_next,
        (
            block_size + w_s * game_loc.nh_b2_const_x,
            block_size * game_loc.nextt_const_y + h_1,
        ),
    )
    screen.blit(
        text_d_score,
...
def draw_board_r(next, hold, score, level, goal, num_of_disrot):
    ...
    if num_of_disrot == 3:
        ...
        screen.blit(
            logo,
            (
                block_size * game_loc.holdt_const_y + 2 * (w_1 - w_3),
                w_3 - block_size * game_loc.rot_help,
            ),
        )
...
def PauseScreen():
    ...
    for event in pygame.event.get():
        ...
        if event.type == QUIT:
            ...
            screen.fill(ui_variables.black)
            screen.blit(pau, (0, 0))
            goto_btn.draw(screen, (0, 0))
            esc_btn.draw(screen, (0, 0))
            ...
            if event.key == K_ESCAPE:
                ...
                if board_state == True:  # 보드 다시 한번 그려주기
                    ...
                    pygame.draw.line(
                        screen,
                        ui_variables.black,
                        [w_4, h_1 * game_loc.d_draw_const],
                        [w_4 + w_1, h_1 * game_loc.d_draw_const],
                        7,
                    )
                    pygame.draw.line(
                        screen,
                        ui_variables.cyan,
                        [w_4 + w_1, h_1 * game_loc.d_draw_const],
                        [w_4 + w_1 * 2, h_1 * game_loc.d_draw_const],
                        7,
                    )
...
while not done:
    ...
    # Start screen
        ...
        origianl_btn.draw(screen, (0, 0, 0))
        rotate_btn.draw(screen, (0, 0, 0))
